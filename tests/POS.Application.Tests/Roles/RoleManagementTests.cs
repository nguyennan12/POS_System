...
public class RoleManagementTests
{
  ...
  [Fact]
  public async Task CreateRole_Success_WhenValid()
  {
    ...
    var handler = new CreateRoleCommandHandler(roleRepository, storeRepository, unitOfWork);
  ...
  public async Task UpdateRole_Success_ForCustomRole()
  {
    ...
    var role = new Role("Old Name", isSystemRole: false, storeId: null, description: "Old desc", id: roleId);
    roleRepository.GetByIdAsync(roleId, Arg.Any<CancellationToken>()).Returns(role);
    roleRepository.ExistsByNameAsync("New Name", null, roleId, Arg.Any<CancellationToken>()).Returns(false);

    var handler = new UpdateRoleCommandHandler(roleRepository, unitOfWork);
  ...
  public async Task UpdateRolePermissions_Success_And_BatchInvalidatesCacheForEmployees()
  {
    ...
    // Verify Batch Invalidation call on Redis Cache with all employee keys
  ...
  public async Task UpdateRolePermissions_Fails_WhenPermissionIdNotFound()
  {
    ...
    // Only 1 permission returned out of 2 requested
  ...
  [Fact]
  public async Task GetRolesQuery_ReturnsRoleList()
  {
    ...
    var handler = new GetRolesQueryHandler(roleRepository);
